...
\subsubsection{Initial chunk state durability}

We say a state is valid if $Pr(b>n-k) < \varepsilon = 2^{-\lambda}$. Since we know that group selection is expressed with the hypergeometric distribution, there are two way to show this:
\begin{enumerate}
    \item Utilizing the generalized hypergeometric function or summation of PMF to derive the CDF. 
    \item Utilizing Hoeffding's Inequality~\cite{Hoeffding1963ProbabilityIF} to draw a bound.
\end{enumerate}
...
\begin{gather}
    ...
    \leq 
    e^{-2(\frac{2}{3}-\frac{k}{n})^{2}n} \\
    \leq \varepsilon
\end{gather}

However, Although the initial state might be valid, we cannot say future states will be. The validity of any state at $T=t$ is evaluated in the next section. 
...
\begin{lemma}
    \label{lemma 1}
    For any group, the probability that it still durable after $T=t$ is bounded by:
    \[
        \sum_{T=1}^{t} (\pi \Theta^{T})_{n-k+1}
    \]
    Where $\pi$ is the initial state matrix and $\Theta$ is a stochastic matrix. 
\end{lemma}



We analyze our system as a Continuous-time Markov chain (CTMC). Expressing our initial group state as a $(n-k+1 \times 1)$ probability vector:
...
We express the probability of reaching any state $T=t$ as $t$ exponentiations of a $(n-k+1 \times n-k+1)$ stochastic matrix $\Theta = (\theta_{i,j})$ on this initial probability vector.
$\theta_{i,j}$ denotes the probability a group with $i$ byzantine nodes at $T=t$ will have $j$ byzantine nodes at $T=t+1$. 

Since we take our group as invalid in perpetuity once it has reached an invalid state; These states are referred to as \textit{absorbing states}. Any other state is considered as an \textit{transient states}. Our matrix is only $(n-k+1 \times n-k+1)$ instead of $(n \times n)$. Furthermore, $\theta_{n-k+1,n-k+1} = 1 \wedge \forall g\in[0,n-k], \theta_{n-k+1,g} = 0$. This handles all transitions from \textit{absorbing} to \textit{absorbing states}. 
...
The first fraction in the second summation (9) thus considers how many byzantine nodes can be evicted. The second fraction (10) then considers the probability of adding back exactly $(j-i+\Upsilon-\upsilon)$ byzantine nodes back into the group to achieve the desired transient state. 
...
\begin{gather}
    \sum_{T=1}^{t} (I \Theta^{T})_{n-k+1} \qquad\blacksquare
\end{gather}

Given enough time, the probability of reaching any state is $1$, therefore, that is to say there is no durability at $T=\infty$. 

However, we can guarantee at some $T=t$, the probability the group has reached an invalid state is still negligible. By extension, it also shows that the probability the group has reached an invalid state for $T=t'|t'\in \mathbb{Z}^{+} \wedge t'<t$ is negligible too. 


\begin{lemma}
    \label{lemma 2}
    For a single data object, the probability that all groups injectively responsible for the $K+R$ chunks being durable at $T=t$ can be bounded by: \[1-(1-\sum_{T=1}^{t} (I \Theta^{T})_{n-k+1})^{K+R} \leq \varepsilon\] Where $I$ is the initial state matrix and $\Theta$ is a stochastic matrix. 
\end{lemma}

By simply extending lemma \ref{lemma 1}; Suppose a data object is made up of $K+R$ chunks, the probability of some group/s reaching an  \textit{absorbing} state is:

\begin{gather}
    1-(1-\sum_{T=1}^{t} (I \Theta^{T})_{n-k+1})^{K+R} \leq \varepsilon \qquad\blacksquare
\end{gather}


\subsection{Durability in the event of targeted attacks}

\subsubsection{Primitives}
Suppose in our system of $\Omega$ data objects, each object is made up of $K+R$ chunks; where only a minimum of $K$ chunks is necessary to reconstruct the original data object. $R$ thus represents the redundancy.
...
\subsubsection{Suppose each physical node only holds multiple fragment}
Since we are drawing an upperbound on the attacker's success probability, we simply have to consider the very worst case. 
If each node can possess $g,g\in \mathbb{Z}^{+}$ fragments, attacking a single node might lead to $g$ groups losing a single honest node each. 
Therefore, we simply change (15) to:
\begin{gather}
    1-
    \left(
    1-\prod_{i=1}^{R}\frac{K+R-i}{\Omega(K+R)-i}
