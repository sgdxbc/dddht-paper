...
Since we take our group as invalid in perpetuity once its reached an invalid state. Our matrix is only $(n-k+1 \times n-k+1)$ instead of $(n \times n)$. Furthemore, $\theta_{n-k+1,n-k+1} = 1 \wedge \forall g\in[0,n-k], \theta_{n-k+1,g} = 0$. This handles all transitions from absorbing to absorbing states. 
...
Furthermore, our system has a eviction rate $\Upsilon$, which denotes the number nodes from all nodes that are selected randomly to be removed from the committee. 
...
\begin{gather*}
    ...
    \Biggl( 
    \sum_{\substack{\upsilon=0\\(j-i+\Upsilon-\upsilon) \geq 0} }^{min(n-i-c,\Upsilon)} 
